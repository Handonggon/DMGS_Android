...
public class HelpNorActivity extends AppCompatActivity {
    ...
    public void handleMessage(Message msg) {
        if  (msg.what == MESSAGE_TIMER_START) {
            ...
            timerhandler.sendEmptyMessageDelayed(MESSAGE_TIMER_START, 10000);
        }
    }
    ...
    private static class TimerHandler extends Handler {
        private final WeakReference<HelpNorActivity> mActivity;
        TimerHandler(HelpNorActivity activity) {
            mActivity = new WeakReference<>(activity);
        }
        @Override
        public void handleMessage(Message msg) {
            HelpNorActivity activity = mActivity.get();
            if (activity != null) {

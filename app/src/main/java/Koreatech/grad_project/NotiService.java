...
//    public static final int MSG_SEND_TO_ACTIVITY = 0;
    ...
    final static int NOTIFICATION_ID_buletooth = 5;
    ...
    final static int LOCATION_DELAY = 50; // 위치검사 시간 간격
    ...
    final static int BLUETOOTH_DELAY = 5; // 블루투스 검사 시간 간격
    private final BluetoothTimerHandler bluetoothTimerHandler = new BluetoothTimerHandler(this);
    final static int BLUETOOTH_TIMER_START = 400;
    ...
    int ImgNumByExhibit;   //각 전시관 사진 갯수
        ...
        if(!infoData.getBoolean("IS_EXHIBITDATA", false)) {      //한번만 실행
            for (int i = 0; i < 6; i++) {
                for (int j = 0; j < ImgNumByExhibit; j++) {
                    String name = infoData.getString("EXHIBIT_" + (i + 1) + "_" + (j + 1) + "_1", "");
                    String mac = infoData.getString("EXHIBIT_" + (i + 1) + "_" + (j + 1) + "_2", "");
                    macMap.put(name, mac);
                }
            }
            SharedPreferences.Editor editor = infoData.edit();
            editor.putBoolean("IS_EXHIBITDATA", true);  //전시물 정보 확정
            editor.apply();
        }
    }
...
//                disconnectedNetwork();
        ...
        if (is_notify) {    // 사용자가 알림 기능을 ON 했으면
            ...
            startForeground(NOTIFICATION_ID, notification); // Foreground로 실행(앱이 꺼져도 서비스가 안죽음)
        ...
        bluetoothTimerHandler.sendEmptyMessage(BLUETOOTH_TIMER_START);
    ...
    public void onDestroy() {
        ...
        bluetoothTimerHandler.removeMessages(BLUETOOTH_TIMER_START);
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (BluetoothDevice.ACTION_FOUND.equals(action)) {
                try {
                    ...
                    Log.d("bluetooth_receiver", "\nName : " + device.getName() + "\nMAC : " + device.getAddress() + "\nRSSI : " + deviceRSSI + "\n");
                    for(Map.Entry<String, String> item : macMap.entrySet()) {
                        if (item.getValue().equals(device.getAddress()) && (Integer.parseInt(deviceRSSI) < 60 && Integer.parseInt(deviceRSSI) > -60)) {
                            makeAlarmNotification("알림", item.getKey() + "이(가) 근처에 있습니다.", NOTIFICATION_ID_buletooth, notificationMainIntent);
                            macMap.remove(item.getKey());
                            break;
                        }
                    }
                }
...
//        return boundary1.contains(gpsPosition) || boundary2.contains(gpsPosition);
        ...
        return true; //boundary.contains(gpsPosition);
                ...
                while (is_initialize) {
                    if (setInitialize()) {
                        if (is_setIsStart) {
                            ...
                            for (int i = 0; i < 6; i++) {
                                editor.remove("IS_CHECK_QR_" + (i + 1));
                                editor.remove("IS_CHECK_PIC_" + (i + 1));
                                for (int j = 0; j < ImgNumByExhibit; j++) {
                                    editor.remove("IS_CHECK_PIC_" + (i + 1) + "-" + (j + 1));
                                    editor.remove("EXHIBIT_" + (i + 1) + "_" + (j + 1) + "_1");
                                    editor.remove("EXHIBIT_" + (i + 1) + "_" + (j + 1) + "_2");
                                    editor.remove("EXHIBIT_" + (i + 1) + "_" + (j + 1) + "_3");
                                    editor.remove("EXHIBIT_" + (i + 1) + "_" + (j + 1) + "_4");
                                }
                            }
                            editor.remove("IS_EXHIBITDATA");
                            editor.apply();
    ...
    public void handleMessage(Message msg) {
        ...
        if (msg.what == BLUETOOTH_TIMER_START) {
            if(!mBluetoothAdapter.isDiscovering()) {
                mBluetoothAdapter.startDiscovery();
            }
            if(!macMap.isEmpty()) {
                bluetoothTimerHandler.sendEmptyMessageDelayed(BLUETOOTH_TIMER_START, BLUETOOTH_DELAY * 1000);
            }
        }
    }
    ...
    // 시간단위 블루투스 확인
    private static class BluetoothTimerHandler extends Handler {
        private final WeakReference<NotiService> mService;
        BluetoothTimerHandler(NotiService service) {
            mService = new WeakReference<>(service);
        }
        @Override
        public void handleMessage(Message msg) {

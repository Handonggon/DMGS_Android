...
Java_Koreatech_grad_1project_OcrActivity_ConvertRGBtoGray(JNIEnv *env, jobject thiz,
                                                              jlong matAddrInput,
                                                              jlong matAddrResult) {
...
extern "C"
JNIEXPORT void JNICALL
Java_Koreatech_grad_1project_CompareActivity_ConvertRGBtoGray(JNIEnv *env, jobject thiz,
                                                                  jlong matAddrInput,
                                                                  jlong matAddrResult) {
    Mat &matInput = *(Mat *)matAddrInput;
    Mat &matResult = *(Mat *)matAddrResult;
...
extern "C"
JNIEXPORT void JNICALL
Java_Koreatech_grad_1project_OcrActivity_warp(JNIEnv *env, jobject thiz,
                                                  jlong input_mat) {
...
extern "C"
JNIEXPORT void JNICALL
Java_Koreatech_grad_1project_OcrActivity_ROI(JNIEnv *env, jobject thiz, jlong input_mat) {
    ocl::setUseOpenCL(true);
    ...
    GaussianBlur(reImg, reImg, Size(5, 5), 1.5, 1.5);

    erode(reImg, reImg, Mat());// these lines may need to be optimized
    dilate(reImg, reImg, Mat());
    dilate(reImg, reImg, Mat());
    erode(reImg, reImg, Mat());

    Canny(reImg, reImg, 50, 150, 3); // canny parameters may need to be optimized

    vector<Point> selected_points;
    vector<vector<Point>> contours;
    Mat hierarchy;

    findContours(reImg, contours, hierarchy, RETR_EXTERNAL, CHAIN_APPROX_SIMPLE);
    sort(contours.begin(), contours.end(), [](const vector<Point>& c1, const vector<Point>& c2) {
        return contourArea(c1, false) > contourArea(c2, false);
    });
    ...
    for (size_t i = 0; i < contours.size(); i++) {
        ...
        if (approx.size() == 4 && isContourConvex(Mat(approx))) {
            line(img, approx[0], approx[approx.size() - 1], Scalar(0, 255, 0), 3);

            for (int k = 0; k < size - 1; k++)
                line(img, approx[k], approx[k + 1], Scalar(0, 255, 0), 3);

            for (int k = 0; k < size; k++)
                circle(img, approx[k], 3, Scalar(0, 0, 255));
